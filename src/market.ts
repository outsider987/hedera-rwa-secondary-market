...
export async function api<T>(path:string,signal:AbortSignal,body?:unknown):Promise<T>{
 const r=await fetch('/api/'+path,{method:body?'POST':'GET',headers:body?{'Content-Type':'application/json'}:undefined,body:body?JSON.stringify(body):undefined,signal:AbortSignal.any([signal,AbortSignal.timeout(10000)]),cache:'no-store',credentials:'omit',redirect:'error'});
