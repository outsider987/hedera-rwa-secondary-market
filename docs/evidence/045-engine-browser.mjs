...
async function capture() {} // Layout unchanged; reuse assertions without new screenshots.
...
try{for(const port of [5173,4173])for(const width of [1440,390]){
 ...
 if(port===4173)await capture(page,`docs/evidence/045-engine-overview-${width===1440?'desktop':'mobile'}.png`);
 ...
 if(port===4173)await capture(page,`docs/evidence/045-engine-market-${width===1440?'desktop':'mobile'}.png`);
 ...
 if(port===4173)await capture(page,`docs/evidence/045-engine-activity-${width===1440?'desktop':'mobile'}.png`);
 ...
 if(port===4173)await capture(page,`docs/evidence/045-engine-settings-${width===1440?'desktop':'mobile'}.png`);
...
writeFileSync('docs/evidence/045-engine-browser.json',JSON.stringify({recordedAt:new Date().toISOString(),kind:'Explicit public fixtures and isolated read-only wallet; no live transactions',results},null,2)+'\n');
