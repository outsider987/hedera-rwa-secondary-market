...
const {chromium}=await import(process.argv[2]),scratch=mkdtempSync(join(tmpdir(),'holdbook-t08-sdk-')),root=process.cwd(),results=[],asset=new Interface(IAsset__factory.abi);
...
try {
 ...
 writeFileSync(join(scratch,'index.html'),'<html lang="en"><title>T08 controlled SDK boundary</title><script type="module" src="/probe.js"></script></html>');
 writeFileSync(join(scratch,'probe.js'),`
 import * as t from './src/lib/trade';import * as settlement from './src/lib/settlement';import * as h from './src/lib/hold';import * as l from './src/lib/lifecycle';import {createAssetProviders} from './src/lib/transport';import {tradeEvidence} from './src/lib/evidence';import {keccak256} from 'viem';
 window.probe=async mode=>{
  ...
  const reversed=mode==='reverse';const st={id:'13-1',contract:input.escrow,salt:'0x'+'7'.repeat(64),status:'Unprepared',baseBlock:input.baseBlock,updatedAt:input.baseTimestamp,digest:'',terms:{matchId:keccak256(new TextEncoder().encode('13-1')),sellerOrder:'0x'+'2'.repeat(64),buyerOrder:'0x'+'3'.repeat(64),seller:reversed?l.accounts.Buyer.address:l.accounts.Seller.address,buyer:reversed?l.accounts.Seller.address:l.accounts.Buyer.address,amount:reversed?'1':'2',priceTinybars:reversed?'9000000':'10000000',preparedAt:input.baseTimestamp,expiry:String(BigInt(input.baseTimestamp)+1800n),holdId:'0'}};const calldata=await settlement.settlementCalldata('lock',st,st.salt),record={schemaVersion:1,chainId:296,kind:'t08-transaction',operationId:'sdk-boundary',startedAt:new Date().toISOString(),action:'lock',status:'awaiting-signature',signerRole:'Seller',input,calldata,calldataDigest:keccak256(calldata),walletValueWeibars:'0'};
  const wallet={provider:{request:async({method,params})=>{
   calls.push(method);if(params[0].data!==calldata||params[0].from.toLowerCase()!==st.terms.seller||params[0].to.toLowerCase()!==l.securityAddress)throw Error('Wrong SDK calldata or parties');
   if(mode==='reject')throw {code:4001};if(mode==='unknown')throw {code:-32603};return '0x'+'1'.repeat(64);
  }}};
  const p=await createAssetProviders({wallet,signer:st.terms.seller,securityAddress:l.securityAddress,calldata:mode==='calldata'?calldata+'00':calldata,reads:await h.holdSdkReads(input),initial:record,sanitize:r=>r,assertContractTransaction:tx=>settlement.assertSettlementTransaction(tx,{action:'lock',sender:st.terms.seller,to:l.securityAddress,calldata:mode==='calldata'?calldata+'00':calldata,value:'0'}),update:r=>updates.push({status:r.status,hasHash:!!r.transactionHash}),checkCurrent:async mutation=>{
  ...
  try {
   await sdk.Network.connect(new sdk.ConnectRequest({network:'testnet',wallet:sdk.SupportedWallets.METAMASK,account:{accountId:reversed?l.accounts.Buyer.accountId:l.accounts.Seller.accountId,evmAddress:st.terms.seller},mirrorNode:{baseUrl:'https://testnet.mirrornode.hedera.com/api/v1/'},rpcNode:{baseUrl:'https://testnet.hashio.io/api',queryProvider:p.read}}),{provider:p.browser});connected=true;
   await Promise.race([settlement.createSettlementHoldSdk(st),new Promise((_,reject)=>timer=setTimeout(()=>{timedOut=true;reject(Error('Harness timeout'));},20000))]);
 ...
 for(const port of [5187,4187])for(const mode of ['reject','hash','reverse','unknown','readonly','stale','calldata','kyc']) {
  ...
  if(['hash','reverse','reject','unknown'].includes(mode)){assert.deepEqual(result.calls,['eth_sendTransaction']);assert.equal(result.status,['hash','reverse'].includes(mode)?'pending':mode==='reject'?'rejected':'unknown');assert.equal(result.hasHash,['hash','reverse'].includes(mode));}
