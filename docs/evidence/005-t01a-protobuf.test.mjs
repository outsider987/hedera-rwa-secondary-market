...
function probe(pkg, name) {
  ...
  if (name === 'truncated-varint') { type = 'AccountID'; input = Buffer.from([0x18, 0x80]); }
    ...
    for (let i = 0; i < depth; i++) input = bytes(6, bytes(1, input));
  ...
  try { proto[type].decode(input); return { outcome: 'accepted', inputBytes: input.length }; }
  catch (error) {
...
if (process.argv.includes('--probe')) {
  ...
  if (process.argv.includes('--candidate')) test('only approved runtime versions are installed', () => {
    ...
    for (const r of resolutions) assert.equal(r.version, '7.6.5');
    assert.equal(lock['node_modules/@grpc/proto-loader/node_modules/protobufjs'].version, '7.6.6');
    result.versionGate = 'passed';
  });
  for (const pkg of protos) {
    for (const [name, expected] of [
      ['compatibility', 'compatible'], ['truncated-varint', 'malformed-rejected'],
      ...
      const child = spawnSync(process.execPath, ['--max-old-space-size=128', file, '--probe', pkg, name], {
        encoding: 'utf8', timeout, killSignal: 'SIGKILL', maxBuffer: 65536,
      });
      let observed;
      ...
      if (name === 'compatibility' && baseline) {
        const old = baseline.cases.find(c => c.package === pkg && c.name === name);
        entry.baselineMatches = JSON.stringify(old?.fixtures) === JSON.stringify(observed.fixtures);
  ...
  after(() => {
    result.securityGate = ['passed', 'not-requested'].includes(result.versionGate)
      && result.cases.length === 22 && result.cases.every(c => c.passed) ? 'passed' : 'failed';
